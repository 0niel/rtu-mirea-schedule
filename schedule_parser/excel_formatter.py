...
class ExcelFormatter(Formatter):
    ...
    def __split_lessons(self, lessons: str) -> list:
        ...
        if len(result) == 0:
            ...
            if ';' in lessons:
                ...
                oneline_lessons_regexp = r'(?:\d+[-,\s.]*)+\s*(?:нед|н)?[.\s+]*\(?(?:кроме|кр)?(?![.\s,\-\d]*(?:подгруппа|подгруп|подгр|п\/г|группа|гр))[.\s]*'
                found = [x for x in re.finditer(oneline_lessons_regexp, lessons)]
                    ...
                    for i in range(1, length):
                        ...
                        if prev_found_pos[1] != current_found_pos[0]:
                            if is_last_elem == False:
                                result.append(lessons[prev_found_pos[0]:current_found_pos[0]])
                            else:
                                result.append(lessons[prev_found_pos[0]:current_found_pos[0]])
                                result.append(lessons[current_found_pos[0]:])
                        else:
                            if is_last_elem == False:
                                result.append(lessons[prev_found_pos[0]:found[i + 1].span()[0]])
                            else:
                                if length == 2:
    ...
    def get_weeks(self, lesson: str, is_even=None, max_weeks=None):
        def get_weeks_list_by_str(weeks_substring, is_even):
            ...
            if '-' in weeks_substring and ',' in weeks_substring:
                ...
                interval_weeks_substring = re.findall(interval_pattern, weeks_substring)[0]
        ...
        for lesson in lessons:
            ...
            exclude_weeks_substr = '' if exclude_weeks_substr is None else exclude_weeks_substr.group(
                4)
    ...
    def get_lessons(self, lesson: str) -> list:      
        def remove_other(lesson):
            ...
            remove_weeks = numbers + r'\s*(?:нед|н)?(?![.\s\d,-]*(?:подгруппа|подгруп|подгр|п\/г|группа|гр))[.\s]*'
        ...
        for i in range(len(result)):
            ...
            if len(types_found) > 0:
                result[i] = {'name': remove_other(result[i]), 'type': types_found[0]}
            else:
