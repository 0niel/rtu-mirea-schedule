...
class CollegeParser(Parser):
        ...
        for day_num in cell_range:
            ...
            for lesson_range in cell_range[day_num]:
                ...
                ready_lessons = self.__get_lessons(
                    lesson_names, lesson_types,
                    lesson_teachers, lesson_rooms, time_
                )
    ...
    def __get_column_range(self, xlsx_sheet, group_name_cell, group_name_row,
                           group_name_row_index):
        ...
        for lesson_num in range(initial_row_num, row_s):
            ...
            if day_num_col.value != '':
                day_num_val = self._get_day_num(
                    day_num_col.value.replace(' ', ''))
            ...
            if re.findall(r'\d+:\d+', str(lesson_time_start_val), flags=re.A):
                lesson_range = (lesson_num_val,
                                {'start': lesson_time_start_val,
