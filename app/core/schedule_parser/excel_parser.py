...
class ExcelParser(Parser):
    ...
    def __get_lessons(self, names, types, teachers, rooms, times, week_num):
        ...
        if lessons_len != 0:
            for i in range(lessons_len):
                ...
                types_by_lesson = (
                    "" if len(types_by_lesson) == 0 else types_by_lesson[0]
                )
                ...
                type_ = (
                    lesson_names[i]["type"]
                    if lesson_names[i]["type"] is not None
                    else types_by_lesson
                )
    ...
    def __read_group_for_semester(
        self, sheet, discipline_col_num, group_name_row_num, cell_range
    ):
        """
        ...
        """
        # Название группы
        group = sheet.cell(group_name_row_num, discipline_col_num).value
            ...
            for lesson_range in cell_range[day_num]:
                try:
                    ...
                    lesson_types = sheet.cell(
                        string_index, discipline_col_num + 1
                    ).value
                    lesson_teachers = sheet.cell(
                        string_index, discipline_col_num + 2
                    ).value
                    lesson_rooms = sheet.cell(
                        string_index, discipline_col_num + 3
                    ).value
        ...
        def get_semester_column_range(
            ...
            for lesson_num in range(initial_row_num, row_s):
                ...
                if lesson_time_col.value != "":
                    lesson_time_start_val = str(lesson_time_col.value).replace("-", ":")
                    lesson_time_start_val = lesson_time_start_val.replace(
                        "09:00", "9:00"
                    )
                ...
                if lesson_time_col.value != "":
                    lesson_time_end_val = str(lesson_time_col.value).replace("-", ":")
                ...
                if re.findall(r"\d+:\d+", str(lesson_time_start_val), flags=re.A):
                    lesson_range = (
                        lesson_num_val,
                        {"start": lesson_time_start_val, "end": lesson_time_end_val},
                        lesson_week_num_val,
                        lesson_string_index,
                    )
                        ...
                        if group not in group_list:
                            ...
                            if self.__doc_type != DOC_TYPE_EXAM:
                                ...
                                one_time_table = self.__read_group_for_semester(
                                    sheet,
                                    group_name_row.index(group_cell),
                                    group_name_row_num,
                                    column_range,
                                )
                                ...
                                save_schedule(
                                    self._db_conn,
                                    one_time_table["group"],
                                    ScheduleModel(**one_time_table["schedule"]),
                                )
